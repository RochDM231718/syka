...
async def search_users(request: Request, query: str, db: Session = Depends(get_db)):
    ...
    return [{"id": u.id, "name": f"{u.first_name} {u.last_name}", "email": u.email, "avatar": u.avatar_path} for u in
            users]


@router.get('/users', response_class=HTMLResponse, name="admin.users.index")
async def index(request: Request, query: Optional[str] = "", page: Optional[int] = 1,
...
async def show(id: int, request: Request, service: UserService = Depends(get_service)):
    ...
    return templates.TemplateResponse('users/show.html', {'request': request, 'user': user, 'roles': list(UserRole)})


@router.post('/users/{id}/role', name='admin.users.update_role')
async def update_role(id: int, request: Request, role: str = Form(...), service: UserService = Depends(get_service)):
...
# --- ОБНОВЛЕННЫЙ МЕТОД UPDATE С ОТЛАДКОЙ ---
...
async def update(
    ...
    try:
        ...
        print(f"DEBUG: Form data keys: {form_data.keys()}")
        avatar_file = form_data.pop('avatar', None)
        print(f"DEBUG: Avatar file object: {avatar_file}")
        if avatar_file:
            print(f"DEBUG: Filename: {getattr(avatar_file, 'filename', 'No filename')}")
        # ----------------------------------

        # Удаляем role из формы
